...
`reader/` is a static reader for article data generated by the parent `kprss`
project.

## Current approach

- Keep the reader implementation isolated under `reader/`.
...
- Hosting should be owned by the parent `kprss` Terraform. The reader keeps the
  app, generator, hosting notes, and manual deploy script under `reader/`.
  Deploy to the existing bucket under `KPRSS_READER_SITE_PREFIX`, usually
  `reader/site`.
