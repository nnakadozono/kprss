...
Lightweight static web reader for news articles collected by `kprss`.
...
Generate the site from the `kprss` SQLite database:
...
From the `kprss` repository root, the same generator can be run as:

```sh
python3 reader/scripts/generate_site.py --out dist
```
